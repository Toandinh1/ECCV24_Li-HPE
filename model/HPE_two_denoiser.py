...
class TwoStageAE(nn.Module):
    def __init__(self, pre_encoder):
        super(TwoStageAE, self).__init__()
...
class TwoLayerDenoiserHPE(nn.Module):
    def __init__(self, encoder):
        super(TwoLayerDenoiserHPE, self).__init__()
        num_lay = 64#numer hidden dim of DyConv1
        hidden_reg = 32 #number hidden dim of Regression
        self.encoder = encoder
        self.skunit1 = SKUnit(in_features=32, mid_features=num_lay, out_features=num_lay, dim1 = 28,dim2 = 10,pool_dim = 'freq-chan', M=1, G=64, r=4, stride=1, L=32)
